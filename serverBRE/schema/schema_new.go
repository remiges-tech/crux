...
func BRESchemaNew(c *gin.Context, s *service.Service) {
	l := s.LogHarbour
	l.Log("Starting execution of BRESchemaNew()")
...
// This is a common check function, we should move this to a util function
...
func verifyPatternSchema(ps types.PatternSchema) []wscutils.ErrorMessage {
	...
	for i, attrSchema := range ps.Attr {
		...
		if !re.MatchString(attrSchema.Name) {
			...
			vErr := wscutils.BuildErrorMessage(
				server.MsgId_Invalid,
				server.ErrCode_Invalid,
				&fieldName,
				attrSchema.Name)
