...
func BRERuleSetNew(c *gin.Context, s *service.Service) {
	...
	err = qtx.WorkFlowNew(c, sqlc.WorkFlowNewParams{
		Realm:      constants.RealmID,
		Slice:      wf.Slice,
		App:        wf.App,
		Brwf:       constants.BRWF,
		Class:      wf.Class,
		Setname:    constants.SetBy,
		Schemaid:   schema.ID,
		IsActive:   pgtype.Bool{Bool: false, Valid: true},
		IsInternal: wf.IsInternal,
		Ruleset:    ruleset,
		Createdby:  constants.SetBy,
	})
	if err != nil {
...
func verifyType(val any, valType string) bool {
	...
	case constants.TypeFloat:
		_, ok = val.(float64)
	case constants.TypeStr, constants.TypeEnum:
		_, ok = val.(string)
	case constants.TypeTS:
		s, _ := val.(string)
		_, err := time.Parse(constants.TimeLayout, s)
		ok = err == nil
	}
	return ok
}
...
func areNextStepAndDoneInProps(props []Property) (bool, bool) {
	...
	for _, p := range props {
		if p.Name == constants.NextStep {
			nsFound = true
		}
		if p.Name == constants.Done && p.Val == constants.TrueStr {
			doneFound = true
		}
	}
