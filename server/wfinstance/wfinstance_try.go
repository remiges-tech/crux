...
func GetWFInstanceTry(c *gin.Context, s *service.Service) {
	...
	isValidReq, errStr := validateWFInstanceNewReq(wfInstancereq, wfinstanceTryreq.Step, realm, s, c)
	if len(errStr) > 0 || !isValidReq {
		...
		wfInstancereq.Entity["step"] = wfinstanceTryreq.Step
		wfInstancereq.Entity["stepfailed"] = strconv.FormatBool(*wfinstanceTryreq.StepFailed)
	}
